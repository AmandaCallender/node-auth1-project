...
router.post('/Register', async (req, res) => {
    ...
    try {
        ...
        if (newLogin) {
            res.status(201).json('We are going on an Adventure!')
...
router.post('/Login', async (req, res) => {
    ...
    try {
        ...
        if (user && bcrypt.compareSync(password, user.password)) {
            ...
            res.status(200).json({ message: `Welcome ${user.username}, we are going on an Adventure!` })
        } else {
    ...
    if (req.session) {
        req.session.destroy((error) => {
            if (error) {
                ...
                res.json({ message: 'Not all those who wander are lost.' })
            }
        })
    } else {
        res.end()
